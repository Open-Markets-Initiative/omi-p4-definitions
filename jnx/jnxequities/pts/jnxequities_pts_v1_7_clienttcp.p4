...
parser JnxequitiesPtsClienttcpParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control JnxequitiesPtsClienttcpVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control JnxequitiesPtsClienttcpIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control JnxequitiesPtsClienttcpEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control JnxequitiesPtsClienttcpComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control JnxequitiesPtsClienttcpDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    JnxequitiesPtsClienttcpParser(),
    JnxequitiesPtsClienttcpVerifyChecksum(),
    JnxequitiesPtsClienttcpIngress(),
    JnxequitiesPtsClienttcpEgress(),
    JnxequitiesPtsClienttcpComputeChecksum(),
    JnxequitiesPtsClienttcpDeparser()
) main;
