// P4_16 (v1model) definition for: Jnx JnxEquities Pts Itch v2.0
// 
// Protocol:
...
header order_added_message_no_attributes_t {
    bit<32> nanoseconds;
    bit<64> order_number;
...
struct headers_t {
    ...
    order_added_message_no_attributes_t order_added_message_no_attributes[MAX_MESSAGES];
...
parser JnxequitiesPtsParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
    ...
    state parse_message {
        ...
        transition select(hdr.message.last.message_type) {
            ...
            8w0x41: parse_order_added_message_no_attributes;
    ...
    state parse_order_added_message_no_attributes {
        packet.extract(hdr.order_added_message_no_attributes.next);
        transition parse_message;
    }
...
control JnxequitiesPtsVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control JnxequitiesPtsIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control JnxequitiesPtsEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control JnxequitiesPtsComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control JnxequitiesPtsDeparser(packet_out packet, in headers_t hdr) {
    apply {
        ...
        packet.emit(hdr.order_added_message_no_attributes);
...
V1Switch(
    JnxequitiesPtsParser(),
    JnxequitiesPtsVerifyChecksum(),
    JnxequitiesPtsIngress(),
    JnxequitiesPtsEgress(),
    JnxequitiesPtsComputeChecksum(),
    JnxequitiesPtsDeparser()
) main;
