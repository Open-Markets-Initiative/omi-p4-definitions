// P4_16 (v1model) definition for: Jnx JnxBonds Pts Itch v1.2
// 
// Protocol:
...
parser JnxbondsPtsParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control JnxbondsPtsVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control JnxbondsPtsIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control JnxbondsPtsEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control JnxbondsPtsComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control JnxbondsPtsDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    JnxbondsPtsParser(),
    JnxbondsPtsVerifyChecksum(),
    JnxbondsPtsIngress(),
    JnxbondsPtsEgress(),
    JnxbondsPtsComputeChecksum(),
    JnxbondsPtsDeparser()
) main;
