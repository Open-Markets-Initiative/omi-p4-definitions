// P4_16 (v1model) definition for: Jnx JnxBonds Pts Ouch v2.01
// 
// Protocol:
...
//   Version: 2.01
//   Date: 1/16/2026
//   Specification: JNX_OUCH_Trading_Specification_Bonds_2.01.pdf
// 
...
header order_executed_with_counter_party_message_t {
    bit<64> timestamp;
    bit<32> order_token;
    bit<32> executed_quantity;
    bit<32> execution_price;
    bit<8> liquidity_indicator;
    bit<96> counter_party;
    bit<64> match_number;
}
...
struct headers_t {
    ...
    order_executed_with_counter_party_message_t order_executed_with_counter_party_message;
    order_rejected_message_t order_rejected_message;
}

parser JnxbondsPtsServerParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
    ...
    state parse_sequenced_data_packet {
        ...
        transition select(hdr.sequenced_data_packet.sequenced_message_type) {
            ...
            8w0x45: parse_order_executed_with_counter_party_message;
    ...
    state parse_order_executed_with_counter_party_message {
        packet.extract(hdr.order_executed_with_counter_party_message);
        transition accept;
    }
...
control JnxbondsPtsServerVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control JnxbondsPtsServerIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control JnxbondsPtsServerEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control JnxbondsPtsServerComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control JnxbondsPtsServerDeparser(packet_out packet, in headers_t hdr) {
    apply {
        ...
        packet.emit(hdr.order_executed_with_counter_party_message);
        packet.emit(hdr.order_rejected_message);
    }
}

V1Switch(
    JnxbondsPtsServerParser(),
    JnxbondsPtsServerVerifyChecksum(),
    JnxbondsPtsServerIngress(),
    JnxbondsPtsServerEgress(),
    JnxbondsPtsServerComputeChecksum(),
    JnxbondsPtsServerDeparser()
) main;
