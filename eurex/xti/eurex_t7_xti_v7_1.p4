...
header best_quote_execution_report_t {
    bit<16> pad2;
...
struct headers_t {
    ...
    best_quote_execution_report_t best_quote_execution_report;
    best_quote_response_t best_quote_response;
...
parser EurexT7XtiParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
        ...
        transition select(hdr.message_header.template_id) {
            ...
            16w10414: parse_best_quote_execution_report;
            16w10413: parse_best_quote_response;
    ...
    state parse_best_quote_execution_report {
        packet.extract(hdr.best_quote_execution_report);
        transition accept;
    }

    state parse_best_quote_response {
        packet.extract(hdr.best_quote_response);
        transition accept;
    }
...
control EurexT7XtiDeparser(packet_out packet, in headers_t hdr) {
    apply {
        ...
        packet.emit(hdr.best_quote_execution_report);
        packet.emit(hdr.best_quote_response);
