...
header otc_upload_broadcast_side_alloc_otc_grp_comp_t {
    bit<8> side;
    bit<8> trading_capacity;
    bit<40> party_executing_firm;
    bit<48> party_executing_trader;
...
header overall_risk_limits_definition_response_t {
    bit<16> pad2;
    bit<64> request_time;
    bit<64> sending_time;
    bit<32> msg_seq_num;
...
header overall_risk_limits_report_t {
    bit<16> pad2;
    bit<64> request_time;
    bit<64> sending_time;
    bit<32> msg_seq_num;
    ...
    bit<64> risk_limit_report_id;
    bit<64> throttle_time_interval;
    bit<64> aggregated_gross_amount;
    bit<64> aggregated_net_amount;
    bit<32> party_id_sponsored_access_unit;
    bit<8> no_risk_limit_types;
    bit<24> pad3;
}

header overall_risk_limits_report_risk_limit_types_grp_comp_t {
...
header rfq_reply_notification_t {
    bit<16> pad2;
    bit<64> trd_reg_ts_time_out;
    bit<64> notification_in;
    bit<64> sending_time;
...
header rfq_specialist_issuer_broadcast_t {
    bit<16> pad2;
    bit<64> trd_reg_ts_time_out;
    bit<64> notification_in;
    bit<64> sending_time;
...
struct metadata_t {
    ...
    bit<8> otc_upload_broadcast_side_alloc_otc_grp_comp_remaining;
    ...
    bit<8> overall_risk_limits_report_risk_limit_types_grp_comp_remaining;
...
struct headers_t {
    ...
    otc_upload_broadcast_side_alloc_otc_grp_comp_t otc_upload_broadcast_side_alloc_otc_grp_comp[MAX_MESSAGES];
    ...
    overall_risk_limits_definition_response_t overall_risk_limits_definition_response;
    overall_risk_limits_report_t overall_risk_limits_report;
    overall_risk_limits_report_risk_limit_types_grp_comp_t overall_risk_limits_report_risk_limit_types_grp_comp[MAX_MESSAGES];
    ...
    rfq_reply_notification_t rfq_reply_notification;
    rfq_reply_response_t rfq_reply_response;
    rfq_response_t rfq_response;
    rfq_specialist_issuer_broadcast_t rfq_specialist_issuer_broadcast;
    reject_t reject;
...
parser EurexT7XtiServerParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
        ...
        transition select(hdr.message_header.template_id) {
            ...
            16w0x7929: parse_otc_response;
            16w0x7a29: parse_otc_upload_broadcast;
            16w0x7827: parse_order_exec_notification;
            16w0x8527: parse_order_exec_report_broadcast;
            16w0x7727: parse_order_exec_response;
            16w0x972a: parse_overall_risk_limits_definition_response;
            16w0x962a: parse_overall_risk_limits_report;
            ...
            16w0xb828: parse_rfq_reply_notification;
            16w0xb728: parse_rfq_reply_response;
            16w0xa228: parse_rfq_response;
            16w0xb328: parse_rfq_specialist_issuer_broadcast;
    ...
    state parse_otc_upload_broadcast {
        ...
        meta.dispatched = 1;
        meta.otc_upload_broadcast_side_alloc_otc_grp_comp_remaining = hdr.otc_upload_broadcast.no_side_otc_allocs;
        transition select(meta.otc_upload_broadcast_side_alloc_otc_grp_comp_remaining) {
            8w0: accept;
            default: parse_otc_upload_broadcast_side_alloc_otc_grp_comp;
        }
    }

    state parse_otc_upload_broadcast_side_alloc_otc_grp_comp {
        packet.extract(hdr.otc_upload_broadcast_side_alloc_otc_grp_comp.next);
        meta.otc_upload_broadcast_side_alloc_otc_grp_comp_remaining = meta.otc_upload_broadcast_side_alloc_otc_grp_comp_remaining - 1;
        transition select(meta.otc_upload_broadcast_side_alloc_otc_grp_comp_remaining) {
            8w0: accept;
            default: parse_otc_upload_broadcast_side_alloc_otc_grp_comp;
        }
    ...
    state parse_overall_risk_limits_definition_response {
        packet.extract(hdr.overall_risk_limits_definition_response);
        meta.dispatched = 1;
        transition accept;
    }

    state parse_overall_risk_limits_report {
        packet.extract(hdr.overall_risk_limits_report);
        meta.dispatched = 1;
        meta.overall_risk_limits_report_risk_limit_types_grp_comp_remaining = hdr.overall_risk_limits_report.no_risk_limit_types;
        transition select(meta.overall_risk_limits_report_risk_limit_types_grp_comp_remaining) {
            8w0: accept;
            default: parse_overall_risk_limits_report_risk_limit_types_grp_comp;
        }
    }

    state parse_overall_risk_limits_report_risk_limit_types_grp_comp {
        packet.extract(hdr.overall_risk_limits_report_risk_limit_types_grp_comp.next);
        meta.overall_risk_limits_report_risk_limit_types_grp_comp_remaining = meta.overall_risk_limits_report_risk_limit_types_grp_comp_remaining - 1;
        transition select(meta.overall_risk_limits_report_risk_limit_types_grp_comp_remaining) {
            8w0: accept;
            default: parse_overall_risk_limits_report_risk_limit_types_grp_comp;
        }
    ...
    state parse_rfq_reply_notification {
        packet.extract(hdr.rfq_reply_notification);
        meta.dispatched = 1;
        transition accept;
    }

    state parse_rfq_reply_response {
        packet.extract(hdr.rfq_reply_response);
        meta.dispatched = 1;
        transition accept;
    }
    ...
    state parse_rfq_specialist_issuer_broadcast {
        packet.extract(hdr.rfq_specialist_issuer_broadcast);
        meta.dispatched = 1;
        transition accept;
    }
...
control EurexT7XtiServerDeparser(packet_out packet, in headers_t hdr) {
    apply {
        ...
        packet.emit(hdr.overall_risk_limits_definition_response);
        packet.emit(hdr.overall_risk_limits_report);
        packet.emit(hdr.overall_risk_limits_report_risk_limit_types_grp_comp);
        ...
        packet.emit(hdr.rfq_reply_notification);
        packet.emit(hdr.rfq_reply_response);
