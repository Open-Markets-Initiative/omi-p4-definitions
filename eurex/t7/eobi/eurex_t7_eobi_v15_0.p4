// P4_16 (v1model) definition for: Eurex T7 Eobi Fbe v15.0
// 
// Protocol:
...
//   Version: 15.0
//   Date: 7/31/2026
//   Specification: T7_R.15.0_ EOBI_Manual_Version_1.pdf
// 
...
header add_complex_instrument_instrmt_leg_grp_comp_t {
    ...
    bit<8> leg_side;
    bit<8> related_price_type;
    bit<8> trading_style;
    bit<64> related_price;
}

header add_flexible_instrument_t {
    bit<64> security_id;
    bit<64> transact_time;
    bit<320> security_desc;
    bit<8> security_type;
    ...
    bit<64> strike_price;
    bit<32> opt_attribute;
    bit<32> contract_date;
    bit<32> security_reference_data_supplement;
    bit<32> pad4;
}

header add_scaled_simple_instrument_t {
    bit<64> security_id;
    bit<64> transact_time;
    bit<320> security_desc;
    bit<8> security_type;
...
header instrument_summary_md_instrument_entry_grp_comp_t {
    ...
    bit<8> md_origin_type;
    bit<8> md_entry_type;
    bit<16> trade_condition;
    bit<16> trd_type;
    bit<8> multi_leg_reporting_type;
    bit<8> multi_leg_price_model;
    bit<64> non_disclosed_trade_volume;
}
...
header mass_instrument_state_change_t {
    ...
    bit<8> tes_security_mass_status;
    bit<64> transact_time;
    bit<8> last_fragment;
...
header tes_trade_report_t {
    ...
    bit<16> number_of_buy_sides;
    bit<16> number_of_sell_sides;
    bit<32> pad4;
}
...
header top_of_book_t {
    ...
    bit<64> bid_size;
    bit<64> offer_size;
    bit<16> number_of_buy_orders;
    bit<16> number_of_sell_orders;
    bit<32> pad4;
}
...
struct headers_t {
    ...
    add_scaled_simple_instrument_t add_scaled_simple_instrument;
    ...
    tes_trade_report_t tes_trade_report;
...
parser EurexT7EobiParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
        ...
        transition select(hdr.packet_header.template_id_2) {
            ...
            16w0x9333: parse_tes_trade_report;
    ...
    state parse_add_flexible_instrument {
        packet.extract(hdr.add_flexible_instrument);
        meta.dispatched = 1;
        transition accept;
    }

    state parse_add_scaled_simple_instrument {
        packet.extract(hdr.add_scaled_simple_instrument);
        meta.dispatched = 1;
        transition accept;
    }
    ...
    state parse_instrument_summary {
        ...
        meta.dispatched = 1;
        meta.instrument_summary_md_instrument_entry_grp_comp_remaining = 16w18;
    ...
    state parse_tes_trade_report {
        packet.extract(hdr.tes_trade_report);
        meta.dispatched = 1;
        transition accept;
    }
    ...
    state parse_trade_reversal {
        ...
        meta.dispatched = 1;
        meta.trade_reversal_md_trade_entry_grp_comp_remaining = 16w18;
...
control EurexT7EobiDeparser(packet_out packet, in headers_t hdr) {
    apply {
        ...
        packet.emit(hdr.add_scaled_simple_instrument);
        ...
        packet.emit(hdr.tes_trade_report);
