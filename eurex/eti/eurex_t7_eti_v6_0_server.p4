// P4_16 (v1model) definition for: Eurex T7 Eti Fbe v6.0
// 
// Protocol:
...
header gateway_response_t {
    ...
    bit<8> gateway_status;
    bit<8> secondary_gateway_status;
    bit<8> session_mode;
    bit<8> trad_ses_mode;
    bit<32> pad4_2;
}
...
header srqs_create_deal_notification_srqs_trd_instrmnt_leg_grp_comp_t {
    bit<64> leg_security_id;
    bit<64> leg_best_bid_px;
    bit<64> leg_best_offer_px;
    bit<32> leg_best_bid_size;
    bit<32> leg_best_offer_size;
}
...
struct metadata_t {
    ...
    bit<8> srqs_create_deal_notification_srqs_trd_instrmnt_leg_grp_comp_remaining;
...
struct headers_t {
    ...
    srqs_create_deal_notification_srqs_trd_instrmnt_leg_grp_comp_t srqs_create_deal_notification_srqs_trd_instrmnt_leg_grp_comp[MAX_MESSAGES];
...
parser EurexT7EtiServerParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    ...
    state parse_srqs_create_deal_notification {
        ...
        meta.dispatched = 1;
        meta.srqs_create_deal_notification_srqs_trd_instrmnt_leg_grp_comp_remaining = hdr.srqs_create_deal_notification.no_legs;
        transition select(meta.srqs_create_deal_notification_srqs_trd_instrmnt_leg_grp_comp_remaining) {
            8w0: accept;
            default: parse_srqs_create_deal_notification_srqs_trd_instrmnt_leg_grp_comp;
        }
    }

    state parse_srqs_create_deal_notification_srqs_trd_instrmnt_leg_grp_comp {
        packet.extract(hdr.srqs_create_deal_notification_srqs_trd_instrmnt_leg_grp_comp.next);
        meta.srqs_create_deal_notification_srqs_trd_instrmnt_leg_grp_comp_remaining = meta.srqs_create_deal_notification_srqs_trd_instrmnt_leg_grp_comp_remaining - 1;
        transition select(meta.srqs_create_deal_notification_srqs_trd_instrmnt_leg_grp_comp_remaining) {
            8w0: accept;
            default: parse_srqs_create_deal_notification_srqs_trd_instrmnt_leg_grp_comp;
        }
...
control EurexT7EtiServerDeparser(packet_out packet, in headers_t hdr) {
    apply {
        ...
        packet.emit(hdr.srqs_create_deal_notification_srqs_trd_instrmnt_leg_grp_comp);
