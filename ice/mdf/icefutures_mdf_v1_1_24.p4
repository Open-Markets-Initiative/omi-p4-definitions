// P4_16 (v1model) definition for: Ice IceFutures Mdf iMpact v1.1.24
// 
// Protocol:
...
struct metadata_t {
    ...
    bit<8> special_field_message_special_field_remaining;
...
struct headers_t {
    ...
    marker_index_prices_t marker_index_prices[MAX_MESSAGES];
...
parser IcefuturesMdfParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    ...
    state parse_packet_header_message {
        ...
        transition select(hdr.packet_header_message.last.message_type) {
            ...
            8w0x7a: parse_marker_index_prices;
    ...
    state parse_marker_index_prices {
        packet.extract(hdr.marker_index_prices.next);
        meta.dispatched = 1;
        transition parse_packet_header_message;
    }
    ...
    state parse_special_field_message {
        ...
        meta.dispatched = 1;
        meta.special_field_message_special_field_remaining = hdr.special_field_message.last.number_of_special_fields;
        transition select(meta.special_field_message_special_field_remaining) {
            8w0: parse_packet_header_message;
    ...
    state parse_special_field_message_special_field {
        ...
        meta.special_field_message_special_field_remaining = meta.special_field_message_special_field_remaining - 1;
        transition select(meta.special_field_message_special_field_remaining) {
            8w0: parse_packet_header_message;
            default: parse_special_field_message_special_field;
        }
...
control IcefuturesMdfDeparser(packet_out packet, in headers_t hdr) {
    apply {
        ...
        packet.emit(hdr.marker_index_prices);
