// P4_16 (v1model) definition for: Ice IceFutures Mdf iMpact v1.1.34
// 
// Protocol:
...
header fixing_indicative_price_message_message_t {
    bit<32> market_id;
    bit<24> iba_currency;
    bit<64> price;
...
header transaction_end_marker_for_empty_last_bundle_message_t {
    bit<64> trade_transaction_id;
}
...
struct headers_t {
    ...
    fixing_indicative_price_message_message_t fixing_indicative_price_message_message[MAX_MESSAGES];
    transaction_end_marker_for_empty_last_bundle_message_t transaction_end_marker_for_empty_last_bundle_message[MAX_MESSAGES];
...
parser IcefuturesMdfParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    ...
    state parse_packet_header_message {
        ...
        transition select(hdr.packet_header_message.last.message_type) {
            ...
            8w0x30: parse_fixing_indicative_price_message_message;
            8w0x65: parse_transaction_end_marker_for_empty_last_bundle_message;
...
control IcefuturesMdfDeparser(packet_out packet, in headers_t hdr) {
    apply {
        ...
        packet.emit(hdr.fixing_indicative_price_message_message);
        packet.emit(hdr.transaction_end_marker_for_empty_last_bundle_message);
