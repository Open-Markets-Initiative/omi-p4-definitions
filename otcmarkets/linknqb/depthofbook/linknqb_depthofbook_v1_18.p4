// P4_16 (v1model) definition for: OtcMarkets LinkNqb DepthOfBook Link v1.18
// 
// Protocol:
...
parser LinknqbDepthofbookParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control LinknqbDepthofbookVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control LinknqbDepthofbookIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control LinknqbDepthofbookEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control LinknqbDepthofbookComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control LinknqbDepthofbookDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    LinknqbDepthofbookParser(),
    LinknqbDepthofbookVerifyChecksum(),
    LinknqbDepthofbookIngress(),
    LinknqbDepthofbookEgress(),
    LinknqbDepthofbookComputeChecksum(),
    LinknqbDepthofbookDeparser()
) main;
