// P4_16 (v1model) definition for: OtcMarkets LinkNqb TopOfBook Link v1.18
// 
// Protocol:
//   Organization: OTC Markets Group
//   Protocol: OTC Top of Book
//   Encoding: Otc Link
//   Version: 1.18
//   Date: 5/1/2026
...
struct headers_t {
    ...
    top_of_book_message_t top_of_book_message[MAX_MESSAGES];
...
parser LinknqbTopofbookParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
    ...
    state parse_top_of_book_message {
        packet.extract(hdr.top_of_book_message.next);
        meta.dispatched = 1;
        transition parse_message;
    }
...
control LinknqbTopofbookVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control LinknqbTopofbookIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
...
control LinknqbTopofbookEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control LinknqbTopofbookComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control LinknqbTopofbookDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    LinknqbTopofbookParser(),
    LinknqbTopofbookVerifyChecksum(),
    LinknqbTopofbookIngress(),
    LinknqbTopofbookEgress(),
    LinknqbTopofbookComputeChecksum(),
    LinknqbTopofbookDeparser()
) main;
