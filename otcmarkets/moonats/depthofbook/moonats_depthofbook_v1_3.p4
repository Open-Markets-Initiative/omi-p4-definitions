// P4_16 (v1model) definition for: OtcMarkets MoonAts DepthOfBook Link v1.3
// 
// Protocol:
...
parser MoonatsDepthofbookParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control MoonatsDepthofbookVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control MoonatsDepthofbookIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control MoonatsDepthofbookEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control MoonatsDepthofbookComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control MoonatsDepthofbookDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    MoonatsDepthofbookParser(),
    MoonatsDepthofbookVerifyChecksum(),
    MoonatsDepthofbookIngress(),
    MoonatsDepthofbookEgress(),
    MoonatsDepthofbookComputeChecksum(),
    MoonatsDepthofbookDeparser()
) main;
