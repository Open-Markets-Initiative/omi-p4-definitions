// P4_16 (v1model) definition for: OtcMarkets MoonAts Retransmission Link v1.3
// 
// Protocol:
...
parser MoonatsRetransmissionParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control MoonatsRetransmissionVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control MoonatsRetransmissionIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control MoonatsRetransmissionEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control MoonatsRetransmissionComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control MoonatsRetransmissionDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    MoonatsRetransmissionParser(),
    MoonatsRetransmissionVerifyChecksum(),
    MoonatsRetransmissionIngress(),
    MoonatsRetransmissionEgress(),
    MoonatsRetransmissionComputeChecksum(),
    MoonatsRetransmissionDeparser()
) main;
