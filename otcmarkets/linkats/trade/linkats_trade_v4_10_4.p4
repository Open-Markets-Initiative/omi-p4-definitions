// P4_16 (v1model) definition for: OtcMarkets LinkAts Trade Link v4.10.4
// 
// Protocol:
...
parser LinkatsTradeParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control LinkatsTradeVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control LinkatsTradeIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control LinkatsTradeEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control LinkatsTradeComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control LinkatsTradeDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    LinkatsTradeParser(),
    LinkatsTradeVerifyChecksum(),
    LinkatsTradeIngress(),
    LinkatsTradeEgress(),
    LinkatsTradeComputeChecksum(),
    LinkatsTradeDeparser()
) main;
