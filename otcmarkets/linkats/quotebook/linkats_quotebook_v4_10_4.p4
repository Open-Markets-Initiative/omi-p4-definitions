// P4_16 (v1model) definition for: OtcMarkets LinkAts QuoteBook Link v4.10.4
// 
// Protocol:
...
parser LinkatsQuotebookParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control LinkatsQuotebookVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control LinkatsQuotebookIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control LinkatsQuotebookEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control LinkatsQuotebookComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control LinkatsQuotebookDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    LinkatsQuotebookParser(),
    LinkatsQuotebookVerifyChecksum(),
    LinkatsQuotebookIngress(),
    LinkatsQuotebookEgress(),
    LinkatsQuotebookComputeChecksum(),
    LinkatsQuotebookDeparser()
) main;
