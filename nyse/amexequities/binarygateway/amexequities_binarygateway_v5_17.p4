// P4_16 (v1model) definition for: Nyse AmexEquities BinaryGateway PillarStream v5.17
// 
// Protocol:
...
parser AmexequitiesBinarygatewayParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control AmexequitiesBinarygatewayVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control AmexequitiesBinarygatewayIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control AmexequitiesBinarygatewayEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control AmexequitiesBinarygatewayComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control AmexequitiesBinarygatewayDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    AmexequitiesBinarygatewayParser(),
    AmexequitiesBinarygatewayVerifyChecksum(),
    AmexequitiesBinarygatewayIngress(),
    AmexequitiesBinarygatewayEgress(),
    AmexequitiesBinarygatewayComputeChecksum(),
    AmexequitiesBinarygatewayDeparser()
) main;
