// P4_16 (v1model) definition for: Nyse AmexOptions TopFeed Pillar v1.2.j
// 
// Protocol:
...
struct headers_t {
    ...
    options_trade_correction_message_t options_trade_correction_message[MAX_MESSAGES];
    options_imbalance_message_t options_imbalance_message[MAX_MESSAGES];
    series_rfq_message_t series_rfq_message[MAX_MESSAGES];
    outright_series_summary_message_t outright_series_summary_message[MAX_MESSAGES];
}
...
parser AmexoptionsTopfeedParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    ...
    state parse_options_trade_correction_message {
        packet.extract(hdr.options_trade_correction_message.next);
        meta.dispatched = 1;
        transition parse_message;
    }
    ...
    state parse_outright_series_summary_message {
        packet.extract(hdr.outright_series_summary_message.next);
        meta.dispatched = 1;
        transition parse_message;
    }
...
control AmexoptionsTopfeedVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control AmexoptionsTopfeedIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
...
control AmexoptionsTopfeedEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control AmexoptionsTopfeedComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}
...
V1Switch(
    AmexoptionsTopfeedParser(),
    AmexoptionsTopfeedVerifyChecksum(),
    AmexoptionsTopfeedIngress(),
    AmexoptionsTopfeedEgress(),
    AmexoptionsTopfeedComputeChecksum(),
    AmexoptionsTopfeedDeparser()
) main;
