// P4_16 (v1model) definition for: Nyse NyseEquities OpenBook Ultra v2.1.b
// 
// Protocol:
...
parser NyseequitiesOpenbookParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control NyseequitiesOpenbookVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control NyseequitiesOpenbookIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control NyseequitiesOpenbookEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control NyseequitiesOpenbookComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control NyseequitiesOpenbookDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    NyseequitiesOpenbookParser(),
    NyseequitiesOpenbookVerifyChecksum(),
    NyseequitiesOpenbookIngress(),
    NyseequitiesOpenbookEgress(),
    NyseequitiesOpenbookComputeChecksum(),
    NyseequitiesOpenbookDeparser()
) main;
