// P4_16 (v1model) definition for: Nyse NyseEquities Bqt Xdp v1.7.a
// 
// Protocol:
...
struct headers_t {
    ...
    consolidated_symbol_clear_message_t consolidated_symbol_clear_message[MAX_MESSAGES];
    ...
    consolidated_stock_summary_message_t consolidated_stock_summary_message[MAX_MESSAGES];
    consolidated_volume_message_t consolidated_volume_message[MAX_MESSAGES];
}
...
parser NyseequitiesBqtParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    ...
    state parse_consolidated_symbol_clear_message {
        packet.extract(hdr.consolidated_symbol_clear_message.next);
        meta.dispatched = 1;
        transition parse_packet_header_message;
    }

    state parse_consolidated_trading_session_change_message {
        packet.extract(hdr.consolidated_trading_session_change_message.next);
        meta.dispatched = 1;
        transition parse_packet_header_message;
    }

    state parse_consolidated_security_status_message {
        packet.extract(hdr.consolidated_security_status_message.next);
        meta.dispatched = 1;
        transition parse_packet_header_message;
    }
    ...
    state parse_bqt_message {
        packet.extract(hdr.bqt_message.next);
        meta.dispatched = 1;
        transition parse_packet_header_message;
    }
    ...
    state parse_consolidated_stock_summary_message {
        packet.extract(hdr.consolidated_stock_summary_message.next);
        meta.dispatched = 1;
        transition parse_packet_header_message;
    }

    state parse_consolidated_volume_message {
        packet.extract(hdr.consolidated_volume_message.next);
        meta.dispatched = 1;
        transition parse_packet_header_message;
    }
