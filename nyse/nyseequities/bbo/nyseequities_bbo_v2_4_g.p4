// P4_16 (v1model) definition for: Nyse NyseEquities Bbo Xdp v2.4.g
// 
// Protocol:
...
header trading_session_change_message_t {
    bit<32> source_time;
    bit<32> source_time_ns;
    bit<32> symbol_index;
    bit<32> symbol_seq_num;
...
struct headers_t {
    ...
    trading_session_change_message_t trading_session_change_message[MAX_MESSAGES];
...
parser NyseequitiesBboParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
    ...
    state parse_trading_session_change_message {
        packet.extract(hdr.trading_session_change_message.next);
        meta.dispatched = 1;
        transition parse_packet_header_message;
    }
...
control NyseequitiesBboVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control NyseequitiesBboIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
...
control NyseequitiesBboEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control NyseequitiesBboComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control NyseequitiesBboDeparser(packet_out packet, in headers_t hdr) {
    apply {
        ...
        packet.emit(hdr.trading_session_change_message);
...
V1Switch(
    NyseequitiesBboParser(),
    NyseequitiesBboVerifyChecksum(),
    NyseequitiesBboIngress(),
    NyseequitiesBboEgress(),
    NyseequitiesBboComputeChecksum(),
    NyseequitiesBboDeparser()
) main;
