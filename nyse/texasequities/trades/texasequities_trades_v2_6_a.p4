// P4_16 (v1model) definition for: Nyse TexasEquities Trades Pillar v2.6.a
// 
// Protocol:
...
struct headers_t {
    ...
    trf_trade_cancel_message_t trf_trade_cancel_message[MAX_MESSAGES];
...
parser TexasequitiesTradesParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
    ...
    state parse_stock_summary_message {
        packet.extract(hdr.stock_summary_message.next);
        meta.dispatched = 1;
        transition parse_message;
    }
    ...
    state parse_trf_trade_cancel_message {
        packet.extract(hdr.trf_trade_cancel_message.next);
        meta.dispatched = 1;
        transition parse_message;
    }
...
control TexasequitiesTradesVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control TexasequitiesTradesIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
...
control TexasequitiesTradesEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control TexasequitiesTradesComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control TexasequitiesTradesDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    TexasequitiesTradesParser(),
    TexasequitiesTradesVerifyChecksum(),
    TexasequitiesTradesIngress(),
    TexasequitiesTradesEgress(),
    TexasequitiesTradesComputeChecksum(),
    TexasequitiesTradesDeparser()
) main;
