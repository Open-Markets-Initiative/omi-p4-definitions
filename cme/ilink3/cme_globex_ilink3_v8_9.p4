...
header quote_cancel_by_group_t {
    bit<64> party_details_list_req_id;
    bit<64> sending_time_epoch;
    bit<8> manual_order_indicator;
    bit<32> seq_num;
...
header quote_cancel_by_instrument_t {
    bit<64> party_details_list_req_id;
    bit<64> sending_time_epoch;
    bit<8> manual_order_indicator;
    bit<32> seq_num;
...
struct headers_t {
    ...
    request_for_cross_t request_for_cross;
    mass_quote_request_t mass_quote_request;
    quote_cancel_by_set_t quote_cancel_by_set;
    quote_cancel_by_group_t quote_cancel_by_group;
    quote_cancel_by_instrument_t quote_cancel_by_instrument;
    mass_quote_response_t mass_quote_response;
}

parser CmeGlobexIlink3Parser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
        ...
        transition select(hdr.message_header.template_id) {
            ...
            16w564: parse_execution_report_pending_cancel;
            16w565: parse_execution_report_pending_replace;
            16w566: parse_request_for_cross;
            16w567: parse_mass_quote_request;
            16w568: parse_quote_cancel_by_set;
            16w569: parse_quote_cancel_by_group;
            16w570: parse_quote_cancel_by_instrument;
            16w571: parse_mass_quote_response;
            default: accept;
        }
    }
    ...
    state parse_request_for_cross {
        packet.extract(hdr.request_for_cross);
        transition accept;
    }
    ...
    state parse_quote_cancel_by_set {
        packet.extract(hdr.quote_cancel_by_set);
        transition accept;
    }

    state parse_quote_cancel_by_group {
        packet.extract(hdr.quote_cancel_by_group);
        transition accept;
    }

    state parse_quote_cancel_by_instrument {
        packet.extract(hdr.quote_cancel_by_instrument);
        transition accept;
    }
...
control CmeGlobexIlink3VerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control CmeGlobexIlink3Ingress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control CmeGlobexIlink3Egress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control CmeGlobexIlink3ComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control CmeGlobexIlink3Deparser(packet_out packet, in headers_t hdr) {
    apply {
        ...
        packet.emit(hdr.request_for_cross);
        packet.emit(hdr.mass_quote_request);
        packet.emit(hdr.quote_cancel_by_set);
        packet.emit(hdr.quote_cancel_by_group);
        packet.emit(hdr.quote_cancel_by_instrument);
        packet.emit(hdr.mass_quote_response);
    }
}

V1Switch(
    CmeGlobexIlink3Parser(),
    CmeGlobexIlink3VerifyChecksum(),
    CmeGlobexIlink3Ingress(),
    CmeGlobexIlink3Egress(),
    CmeGlobexIlink3ComputeChecksum(),
    CmeGlobexIlink3Deparser()
) main;
