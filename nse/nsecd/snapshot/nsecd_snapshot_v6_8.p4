// P4_16 (v1model) definition for: Nse NseCd Snapshot Binary v6.8
// 
// Protocol:
...
parser NsecdSnapshotParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control NsecdSnapshotVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control NsecdSnapshotIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
...
control NsecdSnapshotEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control NsecdSnapshotComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control NsecdSnapshotDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    NsecdSnapshotParser(),
    NsecdSnapshotVerifyChecksum(),
    NsecdSnapshotIngress(),
    NsecdSnapshotEgress(),
    NsecdSnapshotComputeChecksum(),
    NsecdSnapshotDeparser()
) main;
