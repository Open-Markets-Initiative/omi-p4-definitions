// P4_16 (v1model) definition for: Nse NseCom Recovery Binary v7.1
// 
// Protocol:
...
//   Version: 7.1
//   Date: 9/1/2026
//   Specification: MTBT_CO_NNF_PROTOCOL_7.1_20260902170905.pdf
// 
...
header tick_data_recovery_response_message_t {
...
struct headers_t {
    ...
    tick_data_recovery_response_message_t tick_data_recovery_response_message;
}

parser NsecomRecoveryTcpParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
        ...
        transition select(hdr.stream_header.message_type) {
            ...
            8w0x59: parse_tick_data_recovery_response_message;
            default: accept;
        }
    }
    ...
    state parse_tick_data_recovery_response_message {
        packet.extract(hdr.tick_data_recovery_response_message);
        meta.dispatched = 1;
        transition accept;
    }
...
control NsecomRecoveryTcpVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control NsecomRecoveryTcpIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
...
control NsecomRecoveryTcpEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control NsecomRecoveryTcpComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control NsecomRecoveryTcpDeparser(packet_out packet, in headers_t hdr) {
    apply {
        ...
        packet.emit(hdr.tick_data_recovery_response_message);
    }
}

V1Switch(
    NsecomRecoveryTcpParser(),
    NsecomRecoveryTcpVerifyChecksum(),
    NsecomRecoveryTcpIngress(),
    NsecomRecoveryTcpEgress(),
    NsecomRecoveryTcpComputeChecksum(),
    NsecomRecoveryTcpDeparser()
) main;
