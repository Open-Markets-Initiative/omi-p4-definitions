// P4_16 (v1model) definition for: Biva BivaEquities News Itch v1.12
// 
// Protocol:
...
struct headers_t {
    ...
    news_message_t news_message[MAX_MESSAGES];
}

parser BivaequitiesNewsParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
    ...
    state parse_news_message {
        packet.extract(hdr.news_message.next);
        meta.dispatched = 1;
        transition parse_message;
    }
...
control BivaequitiesNewsVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control BivaequitiesNewsIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
...
control BivaequitiesNewsEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control BivaequitiesNewsComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control BivaequitiesNewsDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    BivaequitiesNewsParser(),
    BivaequitiesNewsVerifyChecksum(),
    BivaequitiesNewsIngress(),
    BivaequitiesNewsEgress(),
    BivaequitiesNewsComputeChecksum(),
    BivaequitiesNewsDeparser()
) main;
