...
struct metadata_t {
    bit<1> dispatched;
    bit<8> packet_header_message_remaining;
}
...
parser AquisequitiesUdpheaderParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
        ...
        meta.packet_header_message_remaining = hdr.packet_header.message_count;
        transition select(meta.packet_header_message_remaining) {
            8w0: accept;
    ...
    state parse_packet_header_message {
        ...
        meta.packet_header_message_remaining = meta.packet_header_message_remaining - 1;
        transition select(meta.packet_header_message_remaining) {
            8w0: accept;
            default: parse_packet_header_message;
        }
