// P4_16 (v1model) definition for: Nasdaq PsxEquities TotalView Itch v5.0
// 
// Protocol:
...
header trade_message_non_cross_t {
    bit<16> stock_locate;
    bit<16> tracking_number;
    bit<48> timestamp;
    bit<64> order_reference_number;
...
struct headers_t {
    ...
    trade_message_non_cross_t trade_message_non_cross[MAX_MESSAGES];
...
parser PsxequitiesTotalviewParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
    ...
    state parse_message {
        ...
        transition select(hdr.message.last.message_type) {
            ...
            8w0x50: parse_trade_message_non_cross;
    ...
    state parse_trade_message_non_cross {
        packet.extract(hdr.trade_message_non_cross.next);
        transition parse_message;
    }
...
control PsxequitiesTotalviewVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control PsxequitiesTotalviewIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control PsxequitiesTotalviewEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control PsxequitiesTotalviewComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control PsxequitiesTotalviewDeparser(packet_out packet, in headers_t hdr) {
    apply {
        ...
        packet.emit(hdr.trade_message_non_cross);
...
V1Switch(
    PsxequitiesTotalviewParser(),
    PsxequitiesTotalviewVerifyChecksum(),
    PsxequitiesTotalviewIngress(),
    PsxequitiesTotalviewEgress(),
    PsxequitiesTotalviewComputeChecksum(),
    PsxequitiesTotalviewDeparser()
) main;
