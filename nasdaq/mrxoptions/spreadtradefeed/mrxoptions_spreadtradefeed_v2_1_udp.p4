...
parser MrxoptionsSpreadtradefeedUdpParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control MrxoptionsSpreadtradefeedUdpVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control MrxoptionsSpreadtradefeedUdpIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control MrxoptionsSpreadtradefeedUdpEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control MrxoptionsSpreadtradefeedUdpComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control MrxoptionsSpreadtradefeedUdpDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    MrxoptionsSpreadtradefeedUdpParser(),
    MrxoptionsSpreadtradefeedUdpVerifyChecksum(),
    MrxoptionsSpreadtradefeedUdpIngress(),
    MrxoptionsSpreadtradefeedUdpEgress(),
    MrxoptionsSpreadtradefeedUdpComputeChecksum(),
    MrxoptionsSpreadtradefeedUdpDeparser()
) main;
