...
parser MrxoptionsSpreadordersParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control MrxoptionsSpreadordersVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control MrxoptionsSpreadordersIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control MrxoptionsSpreadordersEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control MrxoptionsSpreadordersComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control MrxoptionsSpreadordersDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    MrxoptionsSpreadordersParser(),
    MrxoptionsSpreadordersVerifyChecksum(),
    MrxoptionsSpreadordersIngress(),
    MrxoptionsSpreadordersEgress(),
    MrxoptionsSpreadordersComputeChecksum(),
    MrxoptionsSpreadordersDeparser()
) main;
