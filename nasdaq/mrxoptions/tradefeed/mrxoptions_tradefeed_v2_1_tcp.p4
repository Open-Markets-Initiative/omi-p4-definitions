...
parser MrxoptionsTradefeedTcpParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control MrxoptionsTradefeedTcpVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control MrxoptionsTradefeedTcpIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control MrxoptionsTradefeedTcpEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control MrxoptionsTradefeedTcpComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control MrxoptionsTradefeedTcpDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    MrxoptionsTradefeedTcpParser(),
    MrxoptionsTradefeedTcpVerifyChecksum(),
    MrxoptionsTradefeedTcpIngress(),
    MrxoptionsTradefeedTcpEgress(),
    MrxoptionsTradefeedTcpComputeChecksum(),
    MrxoptionsTradefeedTcpDeparser()
) main;
