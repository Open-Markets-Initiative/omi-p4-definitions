// P4_16 (v1model) definition for: Nasdaq NtxEquities TotalView Itch v5.0
// 
// Protocol:
...
struct headers_t {
    ...
    retail_interest_message_t retail_interest_message[MAX_MESSAGES];
}

parser NtxequitiesTotalviewParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control NtxequitiesTotalviewVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control NtxequitiesTotalviewIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control NtxequitiesTotalviewEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control NtxequitiesTotalviewComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control NtxequitiesTotalviewDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    NtxequitiesTotalviewParser(),
    NtxequitiesTotalviewVerifyChecksum(),
    NtxequitiesTotalviewIngress(),
    NtxequitiesTotalviewEgress(),
    NtxequitiesTotalviewComputeChecksum(),
    NtxequitiesTotalviewDeparser()
) main;
