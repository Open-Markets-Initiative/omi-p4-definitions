...
parser NtxequitiesOrdersClientParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control NtxequitiesOrdersClientVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control NtxequitiesOrdersClientIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control NtxequitiesOrdersClientEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control NtxequitiesOrdersClientComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control NtxequitiesOrdersClientDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    NtxequitiesOrdersClientParser(),
    NtxequitiesOrdersClientVerifyChecksum(),
    NtxequitiesOrdersClientIngress(),
    NtxequitiesOrdersClientEgress(),
    NtxequitiesOrdersClientComputeChecksum(),
    NtxequitiesOrdersClientDeparser()
) main;
