...
parser NomoptionsIttoClienttcpParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control NomoptionsIttoClienttcpVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control NomoptionsIttoClienttcpIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control NomoptionsIttoClienttcpEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control NomoptionsIttoClienttcpComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control NomoptionsIttoClienttcpDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    NomoptionsIttoClienttcpParser(),
    NomoptionsIttoClienttcpVerifyChecksum(),
    NomoptionsIttoClienttcpIngress(),
    NomoptionsIttoClienttcpEgress(),
    NomoptionsIttoClienttcpComputeChecksum(),
    NomoptionsIttoClienttcpDeparser()
) main;
