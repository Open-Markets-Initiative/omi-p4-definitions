...
parser NomoptionsIttoParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control NomoptionsIttoVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control NomoptionsIttoIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control NomoptionsIttoEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control NomoptionsIttoComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control NomoptionsIttoDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    NomoptionsIttoParser(),
    NomoptionsIttoVerifyChecksum(),
    NomoptionsIttoIngress(),
    NomoptionsIttoEgress(),
    NomoptionsIttoComputeChecksum(),
    NomoptionsIttoDeparser()
) main;
