// P4_16 (v1model) definition for: Nasdaq PhlxOptions DepthOfMarket Itch v1.6
// 
// Protocol:
...
header add_order_short_message_t {
    bit<32> nanoseconds;
...
header add_order_long_message_t {
    bit<32> nanoseconds;
    bit<32> order_reference_number_delta;
    bit<8> market_side;
    bit<32> option_id;
...
header single_side_replace_short_message_t {
    bit<32> nanoseconds;
...
header single_side_replace_long_message_t {
    bit<32> nanoseconds;
    bit<32> original_reference_number_delta;
    bit<32> new_reference_number_delta;
    bit<32> price;
...
header order_replace_short_message_t {
    bit<32> nanoseconds;
    bit<32> original_reference_number_delta;
    bit<32> new_reference_number_delta;
    bit<16> short_price;
...
header order_replace_long_message_t {
    bit<32> nanoseconds;
    bit<32> original_reference_number_delta;
    bit<32> new_reference_number_delta;
    bit<32> price;
...
header quote_replace_short_message_t {
    bit<32> nanoseconds;
...
header quote_replace_long_message_t {
    bit<32> nanoseconds;
    bit<32> original_bid_reference_number_delta;
    bit<32> bid_reference_number_delta;
    bit<32> original_ask_reference_number_delta;
...
struct headers_t {
    ...
    add_order_short_message_t add_order_short_message[MAX_MESSAGES];
    add_order_long_message_t add_order_long_message[MAX_MESSAGES];
    add_quote_short_message_t add_quote_short_message[MAX_MESSAGES];
    add_quote_long_message_t add_quote_long_message[MAX_MESSAGES];
    ...
    single_side_replace_short_message_t single_side_replace_short_message[MAX_MESSAGES];
    single_side_replace_long_message_t single_side_replace_long_message[MAX_MESSAGES];
    order_replace_short_message_t order_replace_short_message[MAX_MESSAGES];
    order_replace_long_message_t order_replace_long_message[MAX_MESSAGES];
    ...
    quote_replace_short_message_t quote_replace_short_message[MAX_MESSAGES];
    quote_replace_long_message_t quote_replace_long_message[MAX_MESSAGES];
    ...
    broken_trade_order_executed_message_t broken_trade_order_executed_message[MAX_MESSAGES];
...
parser PhlxoptionsDepthofmarketParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    ...
    state parse_message {
        ...
        transition select(hdr.message.last.message_type) {
            ...
            8w0x61: parse_add_order_short_message;
            8w0x41: parse_add_order_long_message;
            8w0x6a: parse_add_quote_short_message;
            8w0x4a: parse_add_quote_long_message;
            ...
            8w0x75: parse_single_side_replace_short_message;
            8w0x55: parse_single_side_replace_long_message;
            8w0x76: parse_order_replace_short_message;
            8w0x56: parse_order_replace_long_message;
            8w0x44: parse_single_side_delete_message;
            8w0x47: parse_single_side_update_message;
            8w0x6b: parse_quote_replace_short_message;
            8w0x4b: parse_quote_replace_long_message;
    ...
    state parse_add_order_short_message {
        packet.extract(hdr.add_order_short_message.next);
        transition parse_message;
    }

    state parse_add_order_long_message {
        packet.extract(hdr.add_order_long_message.next);
        transition parse_message;
    }

    state parse_add_quote_short_message {
        packet.extract(hdr.add_quote_short_message.next);
        transition parse_message;
    }

    state parse_add_quote_long_message {
        packet.extract(hdr.add_quote_long_message.next);
        transition parse_message;
    }
    ...
    state parse_single_side_replace_short_message {
        packet.extract(hdr.single_side_replace_short_message.next);
        transition parse_message;
    }

    state parse_single_side_replace_long_message {
        packet.extract(hdr.single_side_replace_long_message.next);
        transition parse_message;
    }

    state parse_order_replace_short_message {
        packet.extract(hdr.order_replace_short_message.next);
        transition parse_message;
    }

    state parse_order_replace_long_message {
        packet.extract(hdr.order_replace_long_message.next);
        transition parse_message;
    }
    ...
    state parse_quote_replace_short_message {
        packet.extract(hdr.quote_replace_short_message.next);
        transition parse_message;
    }

    state parse_quote_replace_long_message {
        packet.extract(hdr.quote_replace_long_message.next);
        transition parse_message;
    }
...
control PhlxoptionsDepthofmarketDeparser(packet_out packet, in headers_t hdr) {
    apply {
        ...
        packet.emit(hdr.add_order_short_message);
        packet.emit(hdr.add_order_long_message);
        packet.emit(hdr.add_quote_short_message);
        packet.emit(hdr.add_quote_long_message);
        ...
        packet.emit(hdr.single_side_replace_short_message);
        packet.emit(hdr.single_side_replace_long_message);
        packet.emit(hdr.order_replace_short_message);
        packet.emit(hdr.order_replace_long_message);
        packet.emit(hdr.single_side_delete_message);
        packet.emit(hdr.single_side_update_message);
        packet.emit(hdr.quote_replace_short_message);
        packet.emit(hdr.quote_replace_long_message);
