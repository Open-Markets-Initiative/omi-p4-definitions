...
parser PhlxoptionsOrdersClienttcpParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control PhlxoptionsOrdersClienttcpVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control PhlxoptionsOrdersClienttcpIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control PhlxoptionsOrdersClienttcpEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control PhlxoptionsOrdersClienttcpComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control PhlxoptionsOrdersClienttcpDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    PhlxoptionsOrdersClienttcpParser(),
    PhlxoptionsOrdersClienttcpVerifyChecksum(),
    PhlxoptionsOrdersClienttcpIngress(),
    PhlxoptionsOrdersClienttcpEgress(),
    PhlxoptionsOrdersClienttcpComputeChecksum(),
    PhlxoptionsOrdersClienttcpDeparser()
) main;
