...
parser PhlxoptionsTradefeedParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control PhlxoptionsTradefeedVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control PhlxoptionsTradefeedIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control PhlxoptionsTradefeedEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control PhlxoptionsTradefeedComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control PhlxoptionsTradefeedDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    PhlxoptionsTradefeedParser(),
    PhlxoptionsTradefeedVerifyChecksum(),
    PhlxoptionsTradefeedIngress(),
    PhlxoptionsTradefeedEgress(),
    PhlxoptionsTradefeedComputeChecksum(),
    PhlxoptionsTradefeedDeparser()
) main;
