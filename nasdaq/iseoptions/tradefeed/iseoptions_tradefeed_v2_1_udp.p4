...
parser IseoptionsTradefeedUdpParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control IseoptionsTradefeedUdpVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control IseoptionsTradefeedUdpIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control IseoptionsTradefeedUdpEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control IseoptionsTradefeedUdpComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control IseoptionsTradefeedUdpDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    IseoptionsTradefeedUdpParser(),
    IseoptionsTradefeedUdpVerifyChecksum(),
    IseoptionsTradefeedUdpIngress(),
    IseoptionsTradefeedUdpEgress(),
    IseoptionsTradefeedUdpComputeChecksum(),
    IseoptionsTradefeedUdpDeparser()
) main;
