...
parser IseoptionsTradefeedClienttcpParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control IseoptionsTradefeedClienttcpVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control IseoptionsTradefeedClienttcpIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control IseoptionsTradefeedClienttcpEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control IseoptionsTradefeedClienttcpComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control IseoptionsTradefeedClienttcpDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    IseoptionsTradefeedClienttcpParser(),
    IseoptionsTradefeedClienttcpVerifyChecksum(),
    IseoptionsTradefeedClienttcpIngress(),
    IseoptionsTradefeedClienttcpEgress(),
    IseoptionsTradefeedClienttcpComputeChecksum(),
    IseoptionsTradefeedClienttcpDeparser()
) main;
