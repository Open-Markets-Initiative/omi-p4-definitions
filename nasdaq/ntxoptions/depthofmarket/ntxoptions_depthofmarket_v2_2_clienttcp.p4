...
parser NtxoptionsDepthofmarketClienttcpParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control NtxoptionsDepthofmarketClienttcpVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control NtxoptionsDepthofmarketClienttcpIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control NtxoptionsDepthofmarketClienttcpEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control NtxoptionsDepthofmarketClienttcpComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control NtxoptionsDepthofmarketClienttcpDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    NtxoptionsDepthofmarketClienttcpParser(),
    NtxoptionsDepthofmarketClienttcpVerifyChecksum(),
    NtxoptionsDepthofmarketClienttcpIngress(),
    NtxoptionsDepthofmarketClienttcpEgress(),
    NtxoptionsDepthofmarketClienttcpComputeChecksum(),
    NtxoptionsDepthofmarketClienttcpDeparser()
) main;
