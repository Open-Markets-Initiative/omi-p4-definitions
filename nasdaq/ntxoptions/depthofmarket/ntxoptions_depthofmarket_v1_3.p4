// P4_16 (v1model) definition for: Nasdaq NtxOptions DepthOfMarket Itch v1.3
// 
// Protocol:
...
header single_side_order_executed_with_price_message_t {
    bit<32> nanoseconds;
    bit<32> reference_number_delta;
    bit<32> cross_number;
    bit<32> match_number;
...
header quote_replace_message_short_form_t {
    ...
    bit<32> ask_reference_delta_number;
    bit<16> bid_price;
    bit<16> bid_size;
    bit<16> ask_price;
    bit<16> ask_size;
...
header block_delete_message_t {
    bit<32> nanoseconds;
    bit<16> total_number_of_reference_number_deltas;
    bit<32> reference_number_deltan;
}
...
header broken_trade_or_order_execution_message_t {
    bit<32> nanoseconds;
    bit<32> cross_number;
    bit<32> match_number;
}
...
struct headers_t {
    ...
    option_open_message_t option_open_message[MAX_MESSAGES];
    ...
    single_side_order_executed_with_price_message_t single_side_order_executed_with_price_message[MAX_MESSAGES];
    ...
    broken_trade_or_order_execution_message_t broken_trade_or_order_execution_message[MAX_MESSAGES];
...
parser NtxoptionsDepthofmarketParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    ...
    state parse_message {
        ...
        transition select(hdr.message_header.last.message_type) {
            ...
            8w0x4f: parse_option_open_message;
            ...
            8w0x43: parse_single_side_order_executed_with_price_message;
            ...
            8w0x42: parse_broken_trade_or_order_execution_message;
    ...
    state parse_option_open_message {
        packet.extract(hdr.option_open_message.next);
        transition parse_message;
    }
    ...
    state parse_single_side_order_executed_with_price_message {
        packet.extract(hdr.single_side_order_executed_with_price_message.next);
        transition parse_message;
    }
    ...
    state parse_broken_trade_or_order_execution_message {
        packet.extract(hdr.broken_trade_or_order_execution_message.next);
        transition parse_message;
    }
...
control NtxoptionsDepthofmarketDeparser(packet_out packet, in headers_t hdr) {
    apply {
        ...
        packet.emit(hdr.option_open_message);
        ...
        packet.emit(hdr.single_side_order_executed_with_price_message);
        ...
        packet.emit(hdr.broken_trade_or_order_execution_message);
