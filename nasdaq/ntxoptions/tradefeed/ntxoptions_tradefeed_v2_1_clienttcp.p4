...
parser NtxoptionsTradefeedClienttcpParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control NtxoptionsTradefeedClienttcpVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control NtxoptionsTradefeedClienttcpIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control NtxoptionsTradefeedClienttcpEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control NtxoptionsTradefeedClienttcpComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control NtxoptionsTradefeedClienttcpDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    NtxoptionsTradefeedClienttcpParser(),
    NtxoptionsTradefeedClienttcpVerifyChecksum(),
    NtxoptionsTradefeedClienttcpIngress(),
    NtxoptionsTradefeedClienttcpEgress(),
    NtxoptionsTradefeedClienttcpComputeChecksum(),
    NtxoptionsTradefeedClienttcpDeparser()
) main;
