...
parser GemxoptionsTradefeedUdpParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control GemxoptionsTradefeedUdpVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control GemxoptionsTradefeedUdpIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control GemxoptionsTradefeedUdpEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control GemxoptionsTradefeedUdpComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control GemxoptionsTradefeedUdpDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    GemxoptionsTradefeedUdpParser(),
    GemxoptionsTradefeedUdpVerifyChecksum(),
    GemxoptionsTradefeedUdpIngress(),
    GemxoptionsTradefeedUdpEgress(),
    GemxoptionsTradefeedUdpComputeChecksum(),
    GemxoptionsTradefeedUdpDeparser()
) main;
