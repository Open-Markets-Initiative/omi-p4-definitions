...
parser GemxoptionsTopofmarketClienttcpParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control GemxoptionsTopofmarketClienttcpVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control GemxoptionsTopofmarketClienttcpIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control GemxoptionsTopofmarketClienttcpEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control GemxoptionsTopofmarketClienttcpComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control GemxoptionsTopofmarketClienttcpDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    GemxoptionsTopofmarketClienttcpParser(),
    GemxoptionsTopofmarketClienttcpVerifyChecksum(),
    GemxoptionsTopofmarketClienttcpIngress(),
    GemxoptionsTopofmarketClienttcpEgress(),
    GemxoptionsTopofmarketClienttcpComputeChecksum(),
    GemxoptionsTopofmarketClienttcpDeparser()
) main;
