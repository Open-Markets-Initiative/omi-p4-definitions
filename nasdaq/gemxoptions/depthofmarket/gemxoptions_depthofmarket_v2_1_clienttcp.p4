...
parser GemxoptionsDepthofmarketClienttcpParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
...
control GemxoptionsDepthofmarketClienttcpVerifyChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control GemxoptionsDepthofmarketClienttcpIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
        standard_metadata.egress_spec = FORWARD_PORT;
    }
}

control GemxoptionsDepthofmarketClienttcpEgress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    apply {
    }
}

control GemxoptionsDepthofmarketClienttcpComputeChecksum(inout headers_t hdr, inout metadata_t meta) {
    apply {
    }
}

control GemxoptionsDepthofmarketClienttcpDeparser(packet_out packet, in headers_t hdr) {
    apply {
...
V1Switch(
    GemxoptionsDepthofmarketClienttcpParser(),
    GemxoptionsDepthofmarketClienttcpVerifyChecksum(),
    GemxoptionsDepthofmarketClienttcpIngress(),
    GemxoptionsDepthofmarketClienttcpEgress(),
    GemxoptionsDepthofmarketClienttcpComputeChecksum(),
    GemxoptionsDepthofmarketClienttcpDeparser()
) main;
