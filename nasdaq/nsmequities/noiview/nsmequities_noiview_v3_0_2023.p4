// P4_16 (v1model) definition for: Nasdaq NsmEquities NoiView Itch v3.0.2023
// 
// Protocol:
...
header direct_listing_with_capital_raise_message_t {
    bit<16> tracking_number;
    bit<48> timestamp;
    bit<64> stock_alpha_8;
...
struct headers_t {
    ...
    direct_listing_with_capital_raise_message_t direct_listing_with_capital_raise_message[MAX_MESSAGES];
}
...
parser NsmequitiesNoiviewParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    ...
    state parse_message {
        ...
        transition select(hdr.message.last.message_type) {
            ...
            8w0x4f: parse_direct_listing_with_capital_raise_message;
            default: accept;
        }
    }
    ...
    state parse_direct_listing_with_capital_raise_message {
        packet.extract(hdr.direct_listing_with_capital_raise_message.next);
        meta.dispatched = 1;
        transition parse_message;
    }
...
control NsmequitiesNoiviewDeparser(packet_out packet, in headers_t hdr) {
    apply {
        ...
        packet.emit(hdr.direct_listing_with_capital_raise_message);
    }
}
