// P4_16 (v1model) definition for: Nasdaq NsmEquities TotalView Itch v5.0.2023
// 
// Protocol:
...
struct headers_t {
    ...
    retail_price_improvement_indicator_message_t retail_price_improvement_indicator_message;
    direct_listing_with_capital_raise_price_discovery_message_t direct_listing_with_capital_raise_price_discovery_message;
}
...
parser NsmequitiesTotalviewServerParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    ...
    state parse_retail_price_improvement_indicator_message {
        packet.extract(hdr.retail_price_improvement_indicator_message);
        meta.dispatched = 1;
        transition accept;
    }

    state parse_direct_listing_with_capital_raise_price_discovery_message {
        packet.extract(hdr.direct_listing_with_capital_raise_price_discovery_message);
        meta.dispatched = 1;
        transition accept;
    }
