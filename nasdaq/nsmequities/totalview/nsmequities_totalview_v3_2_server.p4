// P4_16 (v1model) definition for: Nasdaq NsmEquities TotalView Itch v3.2
// 
// Protocol:
...
header order_replace_message_t {
    bit<96> original_order_reference_number;
    bit<96> new_order_reference_number;
    bit<48> shares_numeric_6;
    bit<80> price;
}
...
parser NsmequitiesTotalviewServerParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
        ...
        transition select(hdr.packet_header.server_packet_type) {
