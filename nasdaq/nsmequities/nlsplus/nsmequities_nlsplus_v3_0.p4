// P4_16 (v1model) definition for: Nasdaq NsmEquities NlsPlus Itch v3.0
// 
// Protocol:
...
header long_form_trade_report_message_t {
    bit<8> originating_market_center_identifier;
    bit<64> issue_symbol;
    bit<8> security_class;
    bit<80> trade_control_number;
...
header long_form_trade_cancel_error_message_t {
    bit<8> originating_market_center_identifier;
    bit<64> issue_symbol;
    bit<8> security_class;
    bit<80> original_trade_control_number;
    bit<64> original_trade_price_long;
    bit<32> original_trade_size;
    bit<8> original_settlement_type;
    bit<8> original_trade_through_exemption;
    bit<8> original_extended_hours_or_sold_code;
...
header next_shares_trade_cancel_error_message_t {
    bit<8> market_center;
    bit<64> issue_symbol;
    bit<8> security_class;
    bit<80> original_trade_control_number;
    bit<32> original_proxy_price;
    bit<32> original_nav_offset_amount;
    bit<32> original_trade_size;
    bit<8> original_settlement_type;
    bit<8> original_trade_through_exemption;
    bit<8> original_extended_hours_or_sold_code;
...
header long_form_trade_correction_message_t {
    bit<8> originating_market_center_identifier;
    bit<64> issue_symbol;
    bit<8> security_class;
    bit<80> original_trade_control_number;
    ...
    bit<8> original_special_sale_condition;
    bit<80> corrected_trade_control_number;
    bit<64> corrected_trade_price_long;
    bit<32> corrected_trade_size;
    bit<8> corrected_settlement_type;
    bit<8> corrected_trade_through_exemption;
    bit<8> corrected_extended_hours_or_sold_code;
...
header next_shares_trade_correction_message_t {
    bit<8> market_center;
    bit<64> issue_symbol;
    bit<8> security_class;
    bit<80> original_trade_control_number;
    ...
    bit<8> original_special_sale_condition;
    bit<80> corrected_trade_control_number;
    bit<32> corrected_proxy_price;
    bit<32> corrected_nav_offset_amount;
    bit<32> corrected_trade_size;
    bit<8> corrected_settlement_type;
    bit<8> corrected_trade_through_exemption;
    bit<8> corrected_extended_hours_or_sold_code;
...
header adjusted_closing_price_message_t {
    bit<64> issue_symbol;
    bit<8> security_class;
    bit<32> adjusted_closing_price;
}

header long_form_adjusted_closing_price_message_t {
    bit<64> issue_symbol;
    bit<8> security_class;
    bit<64> adjusted_closing_price_long;
}
...
header end_of_day_trade_summary_message_t {
    ...
    bit<32> consolidated_closing_price;
    bit<64> consolidated_volume;
    bit<32> consolidated_open_price;
}

header long_form_end_of_day_trade_summary_message_t {
    bit<64> issue_symbol;
    bit<8> market_category;
    bit<64> consolidated_high_price_long;
    bit<64> consolidated_low_price_long;
    bit<64> consolidated_closing_price_long;
    bit<64> consolidated_volume;
    bit<64> consolidated_open_price_long;
}

header next_shares_end_of_day_trade_summary_message_t {
    bit<64> issue_symbol;
    bit<8> market_category;
    bit<32> consolidated_high_price;
    bit<32> nav_offset_amount_high;
    bit<32> consolidated_low_price;
    bit<32> nav_offset_amount_low;
    bit<32> consolidated_closing_price;
    bit<32> nav_offset_amount_closing;
    bit<64> consolidated_volume;
    bit<32> consolidated_open_price;
    bit<32> nav_offset_amount_open;
}
...
struct headers_t {
    ...
    long_form_trade_report_message_t long_form_trade_report_message[MAX_MESSAGES];
    ...
    long_form_trade_cancel_error_message_t long_form_trade_cancel_error_message[MAX_MESSAGES];
    next_shares_trade_cancel_error_message_t next_shares_trade_cancel_error_message[MAX_MESSAGES];
    trade_correction_message_t trade_correction_message[MAX_MESSAGES];
    long_form_trade_correction_message_t long_form_trade_correction_message[MAX_MESSAGES];
    next_shares_trade_correction_message_t next_shares_trade_correction_message[MAX_MESSAGES];
    ...
    long_form_adjusted_closing_price_message_t long_form_adjusted_closing_price_message[MAX_MESSAGES];
    end_of_day_trade_summary_message_t end_of_day_trade_summary_message[MAX_MESSAGES];
    long_form_end_of_day_trade_summary_message_t long_form_end_of_day_trade_summary_message[MAX_MESSAGES];
    next_shares_end_of_day_trade_summary_message_t next_shares_end_of_day_trade_summary_message[MAX_MESSAGES];
...
parser NsmequitiesNlsplusParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    ...
    state parse_message {
        ...
        transition select(hdr.message.last.message_type) {
            ...
            8w0x74: parse_long_form_trade_report_message;
            8w0x4d: parse_next_shares_trade_report_message;
            8w0x58: parse_trade_cancel_error_message;
            8w0x78: parse_long_form_trade_cancel_error_message;
            8w0x4f: parse_next_shares_trade_cancel_error_message;
            8w0x43: parse_trade_correction_message;
            8w0x63: parse_long_form_trade_correction_message;
            8w0x5a: parse_next_shares_trade_correction_message;
            ...
            8w0x67: parse_long_form_adjusted_closing_price_message;
            8w0x4a: parse_end_of_day_trade_summary_message;
            8w0x6a: parse_long_form_end_of_day_trade_summary_message;
            8w0x4e: parse_next_shares_end_of_day_trade_summary_message;
            8w0x49: parse_ipo_information_message;
    ...
    state parse_long_form_trade_report_message {
        packet.extract(hdr.long_form_trade_report_message.next);
        meta.dispatched = 1;
        transition parse_message;
    }
    ...
    state parse_long_form_trade_cancel_error_message {
        packet.extract(hdr.long_form_trade_cancel_error_message.next);
        meta.dispatched = 1;
        transition parse_message;
    }

    state parse_next_shares_trade_cancel_error_message {
        packet.extract(hdr.next_shares_trade_cancel_error_message.next);
        meta.dispatched = 1;
        transition parse_message;
    }
    ...
    state parse_long_form_trade_correction_message {
        packet.extract(hdr.long_form_trade_correction_message.next);
        meta.dispatched = 1;
        transition parse_message;
    }

    state parse_next_shares_trade_correction_message {
        packet.extract(hdr.next_shares_trade_correction_message.next);
        meta.dispatched = 1;
        transition parse_message;
    }
    ...
    state parse_long_form_adjusted_closing_price_message {
        packet.extract(hdr.long_form_adjusted_closing_price_message.next);
        meta.dispatched = 1;
        transition parse_message;
    }
    ...
    state parse_long_form_end_of_day_trade_summary_message {
        packet.extract(hdr.long_form_end_of_day_trade_summary_message.next);
        meta.dispatched = 1;
        transition parse_message;
    }

    state parse_next_shares_end_of_day_trade_summary_message {
        packet.extract(hdr.next_shares_end_of_day_trade_summary_message.next);
        meta.dispatched = 1;
        transition parse_message;
    }
...
control NsmequitiesNlsplusDeparser(packet_out packet, in headers_t hdr) {
    apply {
        ...
        packet.emit(hdr.long_form_trade_report_message);
        packet.emit(hdr.next_shares_trade_report_message);
        packet.emit(hdr.trade_cancel_error_message);
        packet.emit(hdr.long_form_trade_cancel_error_message);
        packet.emit(hdr.next_shares_trade_cancel_error_message);
        packet.emit(hdr.trade_correction_message);
        packet.emit(hdr.long_form_trade_correction_message);
        packet.emit(hdr.next_shares_trade_correction_message);
        ...
        packet.emit(hdr.long_form_adjusted_closing_price_message);
        packet.emit(hdr.end_of_day_trade_summary_message);
        packet.emit(hdr.long_form_end_of_day_trade_summary_message);
        packet.emit(hdr.next_shares_end_of_day_trade_summary_message);
