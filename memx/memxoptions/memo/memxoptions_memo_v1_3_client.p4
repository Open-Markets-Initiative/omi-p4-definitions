// P4_16 (v1model) definition for: Memx MemxOptions Memo Sbe v1.3
// 
// Protocol:
...
header allocation_instruction_message_t {
    ...
    bit<8> alloc_type;
    bit<64> ref_alloc_report_id;
    bit<64> exec_ref_id_optional;
    bit<64> trd_match_id;
    bit<64> security_id;
    bit<8> side;
...
header allocation_instruction_message_allocation_instruction_message_allocations_group_t {
    bit<32> alloc_qty;
    bit<8> trading_capacity;
    bit<8> open_or_close;
    bit<8> block_length_short;
...
struct metadata_t {
    ...
    bit<8> allocation_instruction_message_allocation_instruction_message_allocations_group_remaining;
}
...
struct headers_t {
    ...
    allocation_instruction_message_allocation_instruction_message_allocations_group_t allocation_instruction_message_allocation_instruction_message_allocations_group[MAX_MESSAGES];
}
...
parser MemxoptionsMemoClientParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    ...
    state parse_allocation_instruction_message {
        ...
        meta.dispatched = 1;
        meta.allocation_instruction_message_allocation_instruction_message_allocations_group_remaining = hdr.allocation_instruction_message.num_in_group;
        transition select(meta.allocation_instruction_message_allocation_instruction_message_allocations_group_remaining) {
            8w0: accept;
            default: parse_allocation_instruction_message_allocation_instruction_message_allocations_group;
        }
    }

    state parse_allocation_instruction_message_allocation_instruction_message_allocations_group {
        packet.extract(hdr.allocation_instruction_message_allocation_instruction_message_allocations_group.next);
        meta.allocation_instruction_message_allocation_instruction_message_allocations_group_remaining = meta.allocation_instruction_message_allocation_instruction_message_allocations_group_remaining - 1;
        transition select(meta.allocation_instruction_message_allocation_instruction_message_allocations_group_remaining) {
            8w0: accept;
            default: parse_allocation_instruction_message_allocation_instruction_message_allocations_group;
        }
...
control MemxoptionsMemoClientDeparser(packet_out packet, in headers_t hdr) {
    apply {
        ...
        packet.emit(hdr.allocation_instruction_message_allocation_instruction_message_allocations_group);
    }
}
