// P4_16 (v1model) definition for: Memx MemxOptions MemoirDepth Sbe v1.6.a
// 
// Protocol:
...
struct headers_t {
    ...
    options_instrument_status_message_t options_instrument_status_message[MAX_MESSAGES];
    underlier_instrument_status_message_t underlier_instrument_status_message[MAX_MESSAGES];
    ...
    order_added_short_message_t order_added_short_message[MAX_MESSAGES];
    order_added_long_message_t order_added_long_message[MAX_MESSAGES];
    order_added_extended_message_t order_added_extended_message[MAX_MESSAGES];
...
parser MemxoptionsMemoirdepthParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    ...
    state parse_options_instrument_status_message {
        packet.extract(hdr.options_instrument_status_message.next);
        meta.dispatched = 1;
        transition parse_sequenced_message_message;
    }

    state parse_underlier_instrument_status_message {
        packet.extract(hdr.underlier_instrument_status_message.next);
        meta.dispatched = 1;
        transition parse_sequenced_message_message;
    }
    ...
    state parse_order_added_short_message {
        packet.extract(hdr.order_added_short_message.next);
        meta.dispatched = 1;
        transition parse_sequenced_message_message;
    }

    state parse_order_added_long_message {
        packet.extract(hdr.order_added_long_message.next);
        meta.dispatched = 1;
        transition parse_sequenced_message_message;
    }

    state parse_order_added_extended_message {
        packet.extract(hdr.order_added_extended_message.next);
        meta.dispatched = 1;
        transition parse_sequenced_message_message;
    }
