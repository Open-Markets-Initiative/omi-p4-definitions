// P4_16 (v1model) definition for: Coinbase CoinbaseDerivatives MarketDataApi Sbe v1.9
// 
// Protocol:
...
header outright_instrument_definition_message_t {
    ...
    bit<16> day_of_month;
    bit<32> funding_interval_minutes;
    bit<32> fair_value_limit;
}
...
header start_of_option_instrument_snapshot_message_t {
    bit<16> snapshot_seq_num;
    bit<32> last_instr_seq_num;
    bit<192> symbol;
    bit<64> product_code;
...
struct headers_t {
    ...
    funding_rate_message_t funding_rate_message;
    ...
    start_of_option_instrument_snapshot_message_t start_of_option_instrument_snapshot_message;
...
parser CoinbasederivativesMarketdataapiParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    ...
    state parse_funding_rate_message {
        packet.extract(hdr.funding_rate_message);
        transition accept;
    }
