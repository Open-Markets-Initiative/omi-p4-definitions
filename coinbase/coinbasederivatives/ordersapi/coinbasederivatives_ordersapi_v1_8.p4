// P4_16 (v1model) definition for: Coinbase CoinbaseDerivatives OrdersApi Sbe v1.8
// 
// Protocol:
...
header new_ioc_order_message_t {
    bit<64> client_order_id;
    bit<64> correlation_id;
    bit<64> limit_price;
    bit<32> quantity;
...
header obsolete_stream_order_message_t {
    bit<64> client_order_id;
    bit<64> correlation_id;
...
header event_resend_reject_message_t {
    ...
    bit<440> details_string_55;
}

header reset_options_fill_protection_message_t {
    bit<64> correlation_id;
    bit<32> product_id;
}

header reset_options_fill_protection_ack_message_t {
    bit<64> timestamp;
    bit<64> correlation_id;
    bit<32> product_id;
}

header reset_options_fill_protection_reject_message_t {
    bit<64> timestamp;
    bit<64> correlation_id;
    bit<256> error_message;
}
...
struct headers_t {
    ...
    new_ioc_order_message_t new_ioc_order_message;
    order_entered_message_t order_entered_message;
    replace_order_message_t replace_order_message;
    obsolete_stream_order_message_t obsolete_stream_order_message;
    ...
    reset_options_fill_protection_message_t reset_options_fill_protection_message;
    reset_options_fill_protection_ack_message_t reset_options_fill_protection_ack_message;
    reset_options_fill_protection_reject_message_t reset_options_fill_protection_reject_message;
}
...
parser CoinbasederivativesOrdersapiParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    state start {
        ...
        transition select(hdr.flags.template_id) {
            ...
            16w0x111: parse_new_ioc_order_message;
            16w0x210: parse_order_entered_message;
            16w0x120: parse_replace_order_message;
            16w0x121: parse_obsolete_stream_order_message;
            ...
            16w0x160: parse_reset_options_fill_protection_message;
            16w0x260: parse_reset_options_fill_protection_ack_message;
            16w0x261: parse_reset_options_fill_protection_reject_message;
            default: accept;
        }
    }
    ...
    state parse_new_ioc_order_message {
        packet.extract(hdr.new_ioc_order_message);
        transition accept;
    }
    ...
    state parse_obsolete_stream_order_message {
        packet.extract(hdr.obsolete_stream_order_message);
        transition accept;
    }
    ...
    state parse_reset_options_fill_protection_message {
        packet.extract(hdr.reset_options_fill_protection_message);
        transition accept;
    }

    state parse_reset_options_fill_protection_ack_message {
        packet.extract(hdr.reset_options_fill_protection_ack_message);
        transition accept;
    }

    state parse_reset_options_fill_protection_reject_message {
        packet.extract(hdr.reset_options_fill_protection_reject_message);
        transition accept;
    }
...
control CoinbasederivativesOrdersapiDeparser(packet_out packet, in headers_t hdr) {
    apply {
        ...
        packet.emit(hdr.new_ioc_order_message);
        packet.emit(hdr.order_entered_message);
        packet.emit(hdr.replace_order_message);
        packet.emit(hdr.obsolete_stream_order_message);
        ...
        packet.emit(hdr.reset_options_fill_protection_message);
        packet.emit(hdr.reset_options_fill_protection_ack_message);
        packet.emit(hdr.reset_options_fill_protection_reject_message);
    }
}
